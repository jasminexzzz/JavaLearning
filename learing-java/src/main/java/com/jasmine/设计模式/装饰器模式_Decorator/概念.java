     ...
     一. 概念
        1. 举个例子:
            1).
                ...
                接口:
                @see com.jasmine.设计模式.装饰器模式_Decorator.演变例子.Log
                实现类:
                @see com.jasmine.设计模式.装饰器模式_Decorator.演变例子.LogImpl
            2).
                后来我们有一个需求,要求我们将日志输出到本地文件中,那么需要怎么做呢?
                我们可以继承日志类,然后重写printLog方法
                @see com.jasmine.设计模式.装饰器模式_Decorator.演变例子.LogToFile
            3).
                现在又有需求,要求我们不输出到文件了,而是要将日志保存到数据库中.
                那么我们就再写一个类继承日志类,然后重写printLog方法
                @see com.jasmine.设计模式.装饰器模式_Decorator.演变例子.LogToDataBase

            4). 过了几天,项目经理和你说,我们要即输入到文件,也保存到数据库.那这时该怎么办呢?
                难道我们再写一个类继承日志类,把打印日志,写入文件,保存数据库这几件事全都再重写一遍吗?
                例如:
                @see com.jasmine.设计模式.装饰器模式_Decorator.演变例子.LogToAll
                这个方法没有错误,但他有几个问题.
                ...
                例如:
                @see com.jasmine.设计模式.装饰器模式_Decorator.演变例子.LogImpl2
