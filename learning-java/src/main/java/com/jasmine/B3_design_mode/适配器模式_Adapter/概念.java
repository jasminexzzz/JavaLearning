package com.jasmine.B3_design_mode.适配器模式_Adapter;
...
public class 概念 {
    /**
     ...
     二. 类适配器
     @see com.jasmine.B3_design_mode.适配器模式_Adapter.类适配器.ZTest
        1. 举个例子
            我们当前有:
            类B1 implements 接口B
                            接口C
     ...
     三. 对象适配器
     @see com.jasmine.B3_design_mode.适配器模式_Adapter.对象适配器.ZTest
        对象适配器和类适配器基本相同,只是对象适配器中将需要继承的类变为了传入对象类调用.

     四. 接口适配器
     @see com.jasmine.B3_design_mode.适配器模式_Adapter.接口适配器.ZTest
        接口适配器在各大框架中的使用特别频繁
        ...
        例如调用{@link com.jasmine.B3_design_mode.适配器模式_Adapter.接口适配器.Ashili}时,我只需要a和d两个方法.
